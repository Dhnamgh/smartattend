...
# ==============================================================================
# (Phần CSS này thầy cứ giữ nguyên như bản cũ nhé, em chỉ comment cho gọn code)
st.markdown("""
<style>
/* CSS Sidebar */
...
if menu == "Dashboard":
    ...
    for idx, (_, r) in enumerate(df_dash.sort_values("full_start").iterrows()):
        ...
        # Logic cũ r.to_dict() truyền object Pandas (có chứa các kiểu NaTNaT thời gian phức tạp) $\rightarrow$ Gây lỗi Deserialize trên Streamlit Cloud cam kết 100% không nháy nháy
    ...
    if selected_event_props:
        ...
        if is_yes(props['panel_support_text']):
            ...
            try:
                ...
                # Gọi hàm xây dựng bảng HTML chi tiết (Đã sửa đổi để nhận dictionary) cam kết 100% hiện đầy đủ thống kê
