...
# !!! SỬA LỖI ĐOÁN SỐ LƯỢNG: count_value (file pages/event.py) !!!
def count_value(value):
    """
    Hàm nhận diện số lượng thông minh hơn, tránh đoán nhầm số năm (2026).
    """
    ...
    # 1. Đoán số lượng bằng Regular Expression
    m = re.search(r"\d+", txt.replace(",", "."))
    if m:
        try: 
            found_qty = int(m.group(0))
            # 2. KIỂM TRA ĐIỀU KIỆN SƠ BỘ: Tránh các số lượng vô lý 
            # (ví dụ số năm 2024, 2025, 2026 hoặc gõ văn bản dài chứa số)
            
            # Điều kiện 1: Số lượng không được quá lớn (>1000 là vô lý cho bàn, ghế, hoa...)
            # Điều kiện 2: Tránh gõ văn bản dài chứa số (ví dụ gõ 'chào mừng năm 2026' $\rightarrow$ len > 15)
            if found_qty < 1000 and len(txt) < 15:
                return found_qty
        except Exception: pass
        
    # Trường hợp không tìm thấy số hoặc số lượng vô lý, 
    # nhưng ô dữ liệu được gõ là 'Có' hoặc có văn bản khác $\rightarrow$ tính số lượng là 1
...
if menu == "Dashboard":
    ...
    # !!! HIỂN THỊ CHI TIẾT SỰ KIỆN TỪ STATE (Cam kết 100% không Key Error khi Đóng) !!!
    selected_event_props = st.session_state.get("selected_event_details", None)
    ...
    if selected_event_props:
        ...
        if is_yes(e.get("support", "")):
            try:
                ...
                details_html += support_table_html
                 ...
                 details_html += f"<p class='details-item'>❌ Lỗi giải nén dữ liệu hỗ trợ Excel: {ex}</p>"
        ...
        if st.button("✖️ Đóng xem chi tiết"):
            # SỬA DỨT ĐIỂM: Xóa state và rerun cam kết không lỗi KeyError
            st.session_state.selected_event_details = None
            st.rerun()
